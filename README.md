...
## Публикация собственной копии

GitHub Pages публикует только статические сайты. Это приложение использует Python и Streamlit, поэтому репозиторий можно хранить на GitHub, а само приложение следует запускать на Streamlit Community Cloud или Render.

### Streamlit Community Cloud

1. Создайте новый репозиторий GitHub и отправьте в него проект.
2. На [share.streamlit.io](https://share.streamlit.io/) выберите репозиторий, ветку `main` и файл запуска `web/app.py`.
3. В дополнительных настройках добавьте секрет:

```toml
DADATA_API_KEY = "ваш_новый_ключ"
```

4. Запустите деплой. Без ключа DaData приложение продолжит работать в тестовом режиме.

### Render

В репозитории есть `render.yaml` для Blueprint-деплоя. При создании сервиса Render запросит значение `DADATA_API_KEY`; оно хранится в настройках сервиса и не попадает в Git.

Никогда не добавляйте реальные ключи в `.env.example`, `render.yaml`, исходный код или историю Git.
